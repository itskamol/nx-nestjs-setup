...
export class JwtService {
    ...
    async verifyAccessToken(token: string): Promise<JwtPayload> {
        try {
            ...
            this.logger.warn(`Access token verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    async verifyRefreshToken(token: string): Promise<{ sub: string; type: string }> {
        try {
            ...
            this.logger.warn(`Refresh token verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
